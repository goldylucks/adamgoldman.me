...
class Steps extends Component {
  ...
  renderPromptVariableAc() {
    return (
      <div style={{
 position: 'fixed', top: '50%', left: '50%', zIndex: 1, display: this.state.promptVariableFor ? 'block' : 'none',
}}
      >
        <Typeahead
          ...
          ref={(ref) => { this.elems.variableAc = ref }}
          highlightOnlyResult
  ...
  variableAcOptions() {
    ...
    const stepsVars = this.props.steps.slice(0, this.state.promptVariableFor.sIdx)
      .map((s, sIdx) => ({ ...s, sIdx, isStep: true }))
    const hiddenFieldsVars = this.props.hiddenFields
      .map(h => ({ title: h.label, isHiddenField: true }))
    return hiddenFieldsVars.concat(stepsVars).filter(Boolean)
  }

  variableAcLabelKey = ({
    isStep, isHiddenField, sIdx, title,
  }) => {
    if (isStep) {
      return `${sIdx} - ${title}`
    }
  ...
  variableAcKeyDown = (evt) => {
    ...
    setTimeout(() => {
      this.elems[`childStep-${sIdx}`].elems[key].setSelectionRange(promptIdx + 1, promptIdx + 1)
    }, 0)
  }
  ...
  onSelectVariable = ([selected]) => {
    ...
    this.unpromptVariable(sIdx, key)
    setTimeout(() => {
      this.elems[`childStep-${sIdx}`].elems[key].setSelectionRange(promptIdx + varString.length, promptIdx + varString.length)
    }, 0)
  }
