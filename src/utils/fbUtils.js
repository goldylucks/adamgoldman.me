...
export const fbLinkByUserId = id =>
...
export const getFbShareUrl = path =>
  `https://www.facebook.com/sharer/sharer.php?u=${DOMAIN}${path}`
...
function fbAuth2(response, cb) {
  ...
  global.FB.api(
    '/me?fields=id,name,picture,gender',
    ({ name, picture, gender }) => {
      userServerLogin(
        {
          fbUserId: userID,
          fbClientAccessToken: accessToken,
          name,
          picture,
          gender,
        },
